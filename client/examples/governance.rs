                ...
                match &tx_res {
                    Ok(tx_hash) => {
                        ...
                        match self.node.query_transaction_result(tx_res).await {
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "Reward withdrawal failed during confirmation: {}",
                                    e
                                )
                            }
                        }
            ...
            match &tx_res {
                Ok(tx_hash) => {
                    ...
                    match self.node.query_transaction_result(tx_res).await {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "Affiliate registration failed during confirmation: {}",
                                e
                            )
                        }
                    }
...
async fn main() -> Result<()> {
    ...
    let mut governance_example = governance_example::GovernanceExample::connect().await?;
